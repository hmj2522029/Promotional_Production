...
private:
	...
	Actor2D* CreateAirGround(const TileContext& tile); 	//空中ブロックを生成する関数
	Actor2D* CreateEnemy(const TileContext& tile);		//敵を生成する関数
	Actor2D* CreateSpike(const TileContext& tile);		//トゲを生成する関数
	Actor2D* CreateHole(const TileContext& tile);		//落とし穴を生成する関数
...
public:
	...
	//生成関数
	Actor2D* CreateObject(const TileContext& tileContext);	//マップ記号に対応するタイル生成関数を呼び出す関数
